...
#define S_FUNCTION_NAME SPI_test
/*<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<*/
...
{
    const uint8_T *write = (uint8_T *) ssGetInputPortRealSignal(S, 0);
    ...
    SPI_test_Outputs_wrapper(write, read, spi_test, xD);
}
...
static void mdlUpdate(SimStruct *S, int_T tid)
{
    ...
    SPI_test_Update_wrapper(write, read, spi_test, xD);
}
#endif /* MDL_UPDATE */
